...
for i = 1, _G.NUM_CONTAINER_FRAMES do
	...
	hooksecurefunc("ContainerFrame_GenerateFrame", function(frame, size, id)
		...
		if (size == 1) then
			...
			local baseSize = ContainerFrame_GetContainerNumSlots(id);
    ...
    hooksecurefunc("UpdateContainerFrameAnchors", function()
		...
		local xOffset = containerFrameOffsetX / containerScale;
		...
		for index, frame in ipairs(ContainerFrameSettingsManager:GetBagsShown()) do
			...
			elseif ( freeScreenHeight < frame:GetHeight() ) then
				...
				frame:SetPoint("BOTTOMRIGHT", ContainerFrameSettingsManager:GetBagsShown()[index - 1], "TOPRIGHT", 0, CONTAINER_SPACING);	
			end
